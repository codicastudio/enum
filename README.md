...
```php
use codicastudio\Enum\Rules\EnumValue;
...
```php
use codicastudio\Enum\Rules\EnumKey;
...
```php
use codicastudio\Enum\Rules\Enum;
...
```neon
includes:
- vendor/codicastudio/enum/extension.neon
```
...
array(4) {
  'Administrator' =>
  class codicastudio\Enum\Tests\Enums\UserType#415 (3) {
    public $key =>
  ...
  'Moderator' =>
  class codicastudio\Enum\Tests\Enums\UserType#396 (3) {
    public $key =>
    string(9) "Moderator"
    public $value =>
    int(1)
  ...
  'Subscriber' =>
  class codicastudio\Enum\Tests\Enums\UserType#393 (3) {
    public $key =>
  ...
  'SuperAdministrator' =>
  class codicastudio\Enum\Tests\Enums\UserType#102 (3) {
    public $key =>
